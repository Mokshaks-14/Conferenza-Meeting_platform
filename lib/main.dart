...
String getBackendUrl() {
  // ✅ FIXED: The Correct URL from your screenshot
  return "https://translation-backend-idbs.onrender.com/translate"; 
}
...
class _WebRTCMeetingPageState extends State<WebRTCMeetingPage> {
  ...
  Future<void> _initWebRTC() async {
    ...
    _peerConnection = await createPeerConnection({
      'iceServers': [
        // 1. Google STUN (NO METERED KEYS NEEDED HERE)
  ...
  // --- SAFE MODE MIC: CONTINUOUS LISTENING (Prevents Cutouts) ---
  void _startListening() async {
    bool available = await _speech.initialize(
      ...
      onError: (e) {
        if (mounted) setState(() => _originalText = "Mic Error: ${e.errorMsg}. Use Hotspot.");
        if (_isMicOn) Future.delayed(const Duration(milliseconds: 1000), () => _startListening());
      },
    );

    if (available) {
      if (mounted) setState(() => _originalText = "Listening...");
      _speech.listen(
        localeId: _sourceLang,
        onResult: (res) async {
          String text = res.recognizedWords.trim();
          if (text.isEmpty) return;

          // Send when user pauses
          if (res.finalResult) {
             await FirebaseFirestore.instance.collection('rooms').doc(widget.roomId).collection('captions').add({
        ...
        cancelOnError: true, // IMPORTANT: Cancel on error to restart properly
      );
    } else {
      if (mounted) setState(() => _originalText = "Mic Denied. Click Lock Icon.");
    }
  }

  void _listenToCaptions() {
    _captionsSub = FirebaseFirestore.instance.collection('rooms').doc(widget.roomId).collection('captions')
  ...
  // --- TRANSLATION LOGIC: Show Errors ---
  Future<void> _runTranslation(String text) async {
    try {
      final resp = await http.post(Uri.parse(getBackendUrl()),
          headers: {"Content-Type": "application/json"},
          body: jsonEncode({"text": text, "target_lang": _targetLang, "source_lang": "auto"}));
      ...
      if (resp.statusCode == 200) {
        ...
        if (mounted) setState(() => _translatedText = "Server Error: ${resp.statusCode}");
      }
    } catch (e) {
      print("Translation Error: $e");
      if (mounted) setState(() => _translatedText = "Error: $e");
    }
  }
  ...
  void _toggleMic() {
    ...
    if (_isMicOn && _showTranslation) _startListening();
    else _speech.stop(); 
  }
  ...
  void dispose() {
    ...
    for (var r in _videoRenderers) r.dispose();
              ...
              ? Center(
                  child: GlassCard(
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      const SizedBox(height: 10),
                      const Text("You are viewing the live meeting.", style: TextStyle(color: Colors.white70)),
                      const SizedBox(height: 5),
                      const Text("See translation below.", style: TextStyle(color: Colors.white38, fontSize: 12)),
                    ]),
                  ),
                )
              : PageView.builder(
